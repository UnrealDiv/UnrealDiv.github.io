(function () {
  ...
  const waterMaterial = new THREE.ShaderMaterial({
    ...
    fragmentShader: [
      ...
      "  vec2 uv = vUv;",
      "  uv.x += sin(vUv.y * 5.0 + uTime * 1.45) * 0.045;",
      "  uv.y += cos(vUv.x * 2.2 - uTime * 1.2) * 0.038;",
      "  float sweepA = sin((uv.x * 1.5 + uv.y * 3.2) * 1.0 - uTime * 4.0) * 0.5 + 0.5;",
      "  float sweepB = cos((uv.x * 3.0 - uv.y * 2.1) * 8.0 + uTime * 3.1) * 0.5 + 0.5;",
      "  float rippleA = sin((uv.x + uv.y) * 1.0 - uTime * 5.1) * 0.5 + 0.5;",
      "  float rippleB = sin(length((uv - 0.5) * vec2(1.22, 0.88)) * 36.0 - uTime * 20.0) * 0.5 + 0.5;",
